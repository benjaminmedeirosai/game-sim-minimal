...
function actionsPrompt(): string {
  return [
    'Actions — the steps a plan may contain. Each is ONE line: a keyword then',
    ...
    '    drop = walk to a tile first); pickup collects them back into the bag. Use',
    '    these to gather scattered drops into one place, stock a spot, or lighten a',
    '    unit before a long move.',
    '  - A plain GROUND tile holds only ONE resource, one stack: you cannot pile',
    '    two different items on the same tile — a drop of a second resource spills',
    '    to nearby empty ground. To gather many resources into one spot, use a',
    '    storage depot (below), not bare ground.',
    ...
    '    unit works it from an adjacent tile (it walks there on its own). A depot holds a',
    '    fixed number of stacks (many resource types at once); a partial stack',
    '    still fills a whole slot, so a deposit that would overflow leaves the',
    '    excess in the bag. Depots in sight show their contents in the world',
    '    snapshot.',
    '',
    ...
    '    just pans, "view 40" just zooms, "view AF29 40" does both. The "Player',
